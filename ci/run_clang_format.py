...
if __name__ == "__main__":
    ...
    git_ls_cmd = 'git ls-files | grep -E "(include|src)" | grep -v base | grep -E "\.(c|hpp|cc|cpp|h)$"'
    ...
    if arguments.fix:
        ...
        print("\n".join(map(lambda x: "Formatting {}".format(x),
                            formatted_filenames)))
        subprocess.check_call(["clang-format", "-style={BasedOnStyle: Google, UseCRLF: false, DeriveLineEnding: false, ColumnLimit: 80}",
        ...
        for filename in formatted_filenames:
            ...
            with open(filename, "rb") as reader:
                ...
                formatted = subprocess.check_output(
                    ["clang-format", "-style={BasedOnStyle: Google, UseCRLF: false, DeriveLineEnding: false, ColumnLimit: 80}",
                     filename])
